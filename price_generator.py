...
month_of_travel   = (month_of_purchase + months_in_advance - 1) % 12 + 1
year_of_travel    = 2019 + (month_of_purchase + months_in_advance) // 13
...
price = price * (1 + 1 / months_in_advance)
cond = year_of_travel == 2020
price[cond] = price[cond] * 1.1
cond = np.logical_and(month_of_travel >= 6, month_of_travel <= 8)
price[cond] = price[cond] * 1.3
cond = np.logical_and(month_of_travel >= 11 , month_of_travel <= 12)
...
sample['month_of_travel']   = month_of_travel
sample['year_of_travel']    = year_of_travel
